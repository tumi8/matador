...
echo 1 tunnel >> /etc/iproute2/rt_tables
...
ip route add 10.0.0.8/29 dev wlan0 table tunnel
#ip netns exec tunnel ip r add 10.1.1.0/24 dev veth1
ip netns exec tunnel ip r add 10.0.0.8/29 via 10.1.1.1 dev veth1
...
ip netns exec tunnel iptables --table nat --append POSTROUTING -s 10.0.0.8/29 ! -d 10.0.0.8/29 -j MASQUERADE
