...
A wrapper that needs two country lists with hostnames as input,
builds pairs out of the delta of both lists and starts the controller.
...
Input File: sorted list of Hostnames with informations
    ...
    and
        ...
        AU pl1.eng.monash.edu.au -37.9096 145.133
        CN pl1.6test.edu.cn 40.0 116.327
        HK planetlab1.ie.cuhk.edu.hk 22.3 114.166
...
def execute():
    '''
    The Config file holds the input file names
    Input File: sorted list of Hostnames with informations
        Elements: (CountryCode Hostname Longitude Latitude)
        SortKey: CountryCode
    '''
    ...
    config.read(direc + 'wrapper_delta.ini')

    '''
    A directory were to store the results is given in the log file
    '''
    ...
    '''
    The Input files are read one after anoter and the values are saved in a list:
    country_list = [[country_code, [hostname, longitude, latitude]*]*]
    '''
    number_of_input_files = int(config['general']['number_of_input_files'])

    country_list = []
    tmp_country_list = []
    length_of_each_input = []
    i = 1
    while i <= number_of_input_files:
        file = open(config['input_files']['input'+str(i)])
        country = []
        tmp_country_list = []
        country_code = ' '
        ...
        for line in file:
            helper = line.split()
            #if the country code stays the same the hostname and coordinates get added
            if country_code == helper[0]:
                country = country + [helper[1:4]]
            #if the country code changes, the country gets added to the country list
            else:
                tmp_country_list = tmp_country_list + [country]
                country_code = helper[0]
                country = [helper[0], helper[1:4]]
        #The above algorithm starts with an empty list at the beginning that has to be removed
        file.close()
        tmp_country_list = tmp_country_list[1:] + [country]

        country_list = country_list + tmp_country_list
        length_of_each_input = length_of_each_input + [len(tmp_country_list)]
        i = i+1
    ...
    The matriy gets populated with the following values:
        ...
        2: the country pair was used in an experiment
        ...
        5: Because this wrapper only measures the delta between different input lists, some values
           are already set to 5 before. All value pairs with a five will be skipped later on
    '''
    matrix = [[1 for x in range(dif_countries)] for x in range(dif_countries)]
    counter = 46

    '''
    Seting the already finished pairs to 5
    '''

    base = 0
    i = 0
    j = 0
    for length in length_of_each_input:
        while i < base + length:
            j = base
            while j < base + length:
                matrix[i][j] = 5
    ...
    while i < dif_countries:
        if len(country_list[i]) > 1:
                ...
                if len(country_list[j]) > 1:
                    ...
                    if 0 < matrix[i][j] <= 2:
                        try:
                            ...
                            the counter gets incremented to have a new
                                unique number to store the results
                            ...
                            are set back from 3 to 2, because we want all experiments
                            for one country executed from one hostname.
                            '''
                            message = 'ERROR: ' + hostname1 + ' did not work properly. ' +str(error)
                            message = message+' The the next hostname of this country will be tried'
                            ...
                            '''
                            message = 'ERROR: ' +hostname2 + ' did not work properly. ' + str(error)
                            message = message+' The the next hostname of this country will be tried'
                            write_log(logfile, message)
                            ...
                            '''
                            Fatal Exceptions occur
                            for example when the smartphones don't work correctly.
                            Everything needs to get stopped.
                            '''
                    ...
                    message = message+'\n ERROR: All Nodes of the country ' +str(country_list[j][0])
                    message = message + ' did not work'
                    write_log(logfile, message)
                    k = 0
                    while k < dif_countries:
                        if matrix[k][j] == 1:
            ...
            '''
            When all possible hostnames of a country failed
            a warning is writtenand
            every value 1 in the matrix containing this country  is set to 0.
            '''
